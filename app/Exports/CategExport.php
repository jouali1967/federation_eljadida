...
class CategExport implements FromArray, WithHeadings, WithEvents
{
  protected $categorie;
  
  protected $employes;

  public function __construct($employes, $categorie)
  {
  ...
  {
    return [
      ['Liste des employés categorie : '.$this->categorie],
      [],
      ['N°', 'Nom et Prénom', 'Fonction', 'N° CNSS', 'CIN', 'Date Embauche', 'Telephone'],
    ];
  }
    ...
    return [
      AfterSheet::class => function (AfterSheet $event) {
        ...
        foreach ($this->headings() as $heading) {
          $headerCount++;
          if ($heading === ['N°', 'Nom et Prénom', 'Fonction','N° CNSS', 'CIN', 'Date Embauche', 'Telephone']) {
            break;
          }
        }
        ...
        $event->sheet->getStyle('A1:G2')->getAlignment()->setHorizontal(\PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_CENTER);
        $event->sheet->getStyle('A1:G2')->getFont()->setBold(true);

        // Centrer les libellés du tableau (ligne d'en-tête)
        $event->sheet->getStyle('A' . $tableStart . ':G' . $tableStart)
        ...
        $event->sheet->getStyle('B' . ($tableStart + 1) . ':G' . $tableEnd)
          ->getAlignment()->setHorizontal(\PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_LEFT);

        // Bordures sur tout le tableau
        $event->sheet->getStyle('A' . $tableStart . ':G' . $tableEnd)
          ->applyFromArray([
            'borders' => [
              'allBorders' => [
